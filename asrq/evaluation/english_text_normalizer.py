...
# File Source: https://github.com/huggingface/open_asr_leaderboard/blob/main/normalizer/normalizer.py
        ...
        for i, current in enumerate(words):
            ...
            if re.match(r"^\d+(\.\d+)?$", current_without_prefix):
                ...
                if value is None:
                    ...
                    if f is not None and p.denominator == 1: # type: ignore
                        value = p.numerator # type: ignore
                    else:
                        yield output(value)
                        value = multiplier
                else:
                ...
                if value is None:
                    ...
                    p = f * multiplier if f is not None else None
                    if f is not None and p.denominator == 1: # type: ignore
                        yield output(str(p.numerator) + suffix) # type: ignore
                    else:
                        yield output(value)
                        yield output(str(multiplier) + suffix)
                else:  # int
