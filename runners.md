...
print("Saved alternating rotation to:", transform.cfg.path)
PY
'
...
python -m asrq.exp \
  ...
  transform.search_mode=alternating \
  transform.batch_size = 64 \
  transform.path=/home/blessedg/asrq/outputs/rotation/openai-whisper-large-v3_rotation_w4a8_search.pt \
  evaluate=true
'

'''
1. Whisper Search
2. Global Search for R1 after R2.
3. Parallelize the layer wise search
4. Time the search
5. Randomize start point.
6. Increase the num samples.
7. Skip  Q2 if Qe doesnt change meaningfully.
8. Alternating Q2 -> Qe -> Q2 search is supported for Whisper and Parakeet.
'''
